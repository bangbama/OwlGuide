        ...
        .popover(
            ...
            VStack(alignment: .leading, spacing: 12) {
                ...
                HStack(spacing: 12) {
                        ...
                        if !isListening {
                            SpeechManager.shared.requestPermissions { granted, speechGranted, micGranted in
                                if granted {
                                    ...
                                    if !speechGranted { missingPermissions.append("语音识别") }
                                    if !micGranted { missingPermissions.append("麦克风") }
                                    
                                    let alert = NSAlert()
                                    alert.messageText = "需要\(missingPermissions.joined(separator: "和"))权限"
                                    alert.informativeText = "请在调试面板中开启相关权限，或到系统设置 > 隐私与安全性中手动开启"
                                    alert.addButton(withTitle: "打开调试面板")
                    ...
                    .accessibilityLabel(isListening ? "正在录音，点击停止" : "语音输入")
                    .accessibilityHint("点击开始语音输入，再次点击停止，停止后会自动提交查询")
